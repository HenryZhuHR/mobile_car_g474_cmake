...
void Encoder_Task(uint32_t dT_us)
{
 ...
 * @param  dT_us            控制周期
 */
void Motor_Task(uint32_t dT_us)
{
...
void Exp_Speed_Cal(uint32_t dT_us)
{
	...
	// 当用姿态传感器测量得到的z轴角速度和用编码器数据解算得到的角速度相差很大时，可认为轮子打滑
	// if ((kinematics.exp_vel.linear_x == 0 && kinematics.exp_vel.linear_y == 0 && kinematics.exp_vel.angular_z == 0) || my_abs(sensor.gyro_rps[Z] - kinematics.fb_vel.angular_z) > 0.5)
	// {
 ...
 * @param  dT_us            计算周期
 */
void Fb_Speed_Cal(uint32_t dT_us)
{
	float average_rps_x;
	float average_rps_y;
	float average_rps_a;
 ...
 * @param  inte_lim         积分限幅
 */
void PID_Controller(uint32_t dT_us,
					float expect,
					float feedback,
					pid_st *pid,
					float inte_d_lim,
					float inte_lim)
{
	...
	// 比例、积分、微分相加
	pid->out += pid->kp * pid->err + pid->ki * pid->err_inte + pid->kd * pid->err_diff;
